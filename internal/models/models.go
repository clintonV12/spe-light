...
// IsPlatformRole returns true for roles that have cross-org visibility.
func (r Role) IsPlatformRole() bool {
	return r == RoleSuperAdmin || r == RolePlatformSupport
}

// CanEditPlans returns true for roles that can create/edit plans.
...
// ── TokenClaims — embedded in JWT ─────────────────────────────────────────

// TokenClaims are the values issued inside access tokens and returned by
// ParseAccessToken. OrgID is nil for platform-tier users (super_admin,
// platform_support) who are not scoped to a single org.
...
// Activity is a discrete unit of work within a phase.
