  ...
  return (
    <div className="space-y-2">
      <div className="grid grid-cols-1 gap-2.5 lg:grid-cols-5">
        {block('key_partners', <Handshake className="size-3.5" />, 'Key Partners',
          'border-ink-200 bg-ink-50', 'lg:col-start-1 lg:row-start-1 lg:row-span-2')}

        {block('key_activities', <Zap className="size-3.5" />, 'Key Activities',
          'border-p1 bg-p1-light', 'lg:col-start-2 lg:row-start-1')}
        {block('key_resources', <Boxes className="size-3.5" />, 'Key Resources',
          'border-p1 bg-p1-light', 'lg:col-start-2 lg:row-start-2')}

        {block('value_propositions', <Gem className="size-3.5" />, 'Value Propositions',
          'border-accent bg-accent-50', 'lg:col-start-3 lg:row-start-1 lg:row-span-2')}

        {block('customer_relationships', <Heart className="size-3.5" />, 'Customer Relationships',
          'border-p2 bg-p2-light', 'lg:col-start-4 lg:row-start-1')}
        {block('channels', <Truck className="size-3.5" />, 'Channels',
          'border-p2 bg-p2-light', 'lg:col-start-4 lg:row-start-2')}

        {block('customer_segments', <Users className="size-3.5" />, 'Customer Segments',
          'border-p3 bg-p3-light', 'lg:col-start-5 lg:row-start-1 lg:row-span-2')}
      </div>

      <div className="grid grid-cols-1 gap-2.5 lg:grid-cols-5">
        {block('cost_structure', <Receipt className="size-3.5" />, 'Cost Structure',
          'border-red-200 bg-red-50', 'lg:col-start-1 lg:col-span-2')}
        {block('revenue_streams', <DollarSign className="size-3.5" />, 'Revenue Streams',
