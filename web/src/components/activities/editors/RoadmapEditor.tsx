...
const QUARTERS: { key: keyof RoadmapContent; label: string; color: string; dot: string }[] = [
  { key: 'q1', label: 'Q1', color: 'border-p1 bg-p1-light', dot: 'bg-p1' },
  { key: 'q2', label: 'Q2', color: 'border-p2 bg-p2-light', dot: 'bg-p2' },
  { key: 'q3', label: 'Q3', color: 'border-p3 bg-p3-light', dot: 'bg-p3' },
  { key: 'q4', label: 'Q4', color: 'border-accent bg-accent-50', dot: 'bg-accent' },
]
  ...
  return (
    <div className="relative">
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-4">
        {QUARTERS.map(({ key, label, color, dot }) => (
          <div key={key} className="relative">
            ...
            <div className={`flex h-full flex-col rounded-xl border-2 p-3 ${color}`}>
              <div className="mb-1.5 flex items-center gap-1.5">
                ...
                <p className="text-xs font-bold uppercase tracking-wide text-ink-600">{label} Milestones</p>
              </div>
              <textarea
                className="min-h-28 w-full flex-1 resize-none bg-transparent text-sm text-ink-800 outline-none placeholder:text-ink-400"
                placeholder={`What ships or happens in ${label}?`}
                value={content[key]}
